...
- **Risoluzione Modelli Sovraccarichi (503/429)**: Modificata la Edge Function `gemini-proxy` per introdurre un meccanismo di fallback incrociato generico (`['gemini-1.5-flash', 'gemini-1.5-pro', 'gemini-2.0-flash-exp']`). Se il modello di default (Flash) fallisce a causa del sovraccarico temporaneo dei server Google (`503 Service Unavailable`), l'applicazione passa automaticamente ed in modo trasparente al modello Pro (e viceversa), escludendo loop di tentativi su alias equivalenti (es. `gemini-flash-latest` -> `gemini-1.5-flash`).
