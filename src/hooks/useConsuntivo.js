    ...
    try {
      ...
      if (titolareId) {
        const { data: prof } = await supabase
          .from('profiles')
          .select('studio_nome, studio_indirizzo, studio_telefono, studio_email, studio_pec, logo_base64, ragione_sociale, partita_iva, codice_fiscale')
          .eq('id', titolareId).maybeSingle()
        branding = prof || null
      }

      // 10) template attivo dell'amministratore titolare
      let tmpl = { ...DEFAULT_TEMPLATE, tipo_modello: 'condofast', nome: 'Modello Standard CondoFAST' }
      if (titolareId) {
        ...
        if (ct?.struttura && Object.keys(ct.struttura).length) {
          ...
          if (tipo === 'condofast' || tipo === 'condosmart') {
            tmpl = {
              ...DEFAULT_TEMPLATE,
              id: ct.id,
