...
function buildFoglioAnagrafica(ws, unita) {
  ...
  unita.forEach((u, i) => {
    ...
    const row = ws.addRow({
      ...
      dal: inq ? fmtData(inq.data_inizio) : '', al: inq ? fmtData(inq.data_fine) : '',
    });
    styleDataRow(row, i);
  });
}
...
function buildFoglioRate(ws, rate, cells, unita, getProprietario) {
  ...
  let i = 0;
  (unita || []).forEach(u => {
    const prop = getProprietario ? getProprietario(u) : (u.occupanti?.find(o => o.tipo_occupante === 'proprietario'));
