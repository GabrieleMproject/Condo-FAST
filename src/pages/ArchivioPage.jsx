...
export default function ArchivioPage() {
  ...
  // Render diff tra dati_prima e dati_dopo
          ...
          {cambiati.map(k => (
            <div key={k} style={{ marginBottom: 6, background: '#0f172a', borderRadius: 6, padding: '6px 10px' }}>
              ...
              <div style={{ display: 'flex', gap: 8, alignItems: 'flex-start' }}>
                <span style={{
                  background: '#ef444422', color: '#f87171', padding: '2px 6px',
                  borderRadius: 4, maxWidth: '45%', wordBreak: 'break-all'
                }}>
                  {JSON.stringify(prima[k]) ?? 'null'}
                </span>
                <span style={{ color: '#475569' }}>→</span>
                <span style={{
                  background: '#10b98122', color: '#10b981', padding: '2px 6px',
                  borderRadius: 4, maxWidth: '45%', wordBreak: 'break-all'
                }}>
  ...
  return (
    <div style={{ padding: 32, maxWidth: 1200, margin: '0 auto', fontFamily: 'Sora, sans-serif' }}>
      ...
      <div style={{
        ...
        <div style={{ flex: '2 1 200px' }}>
          ...
          <input
            ...
            placeholder="Tabella, note..."
            value={filtri.cerca}
            onChange={e => setFiltro('cerca', e.target.value)}
          />
        </div>
        ...
        <div style={{ flex: '2 1 180px' }}>
          ...
          <select style={{ ...inputStyle, width: '100%' }}
            value={filtri.condominioId} onChange={e => setFiltro('condominioId', e.target.value)}>
        ...
        <div style={{ flex: '1 1 140px' }}>
          ...
          <select style={{ ...inputStyle, width: '100%' }}
            value={filtri.categoria} onChange={e => setFiltro('categoria', e.target.value)}>
        ...
        <div style={{ flex: '1 1 130px' }}>
          ...
          <select style={{ ...inputStyle, width: '100%' }}
            value={filtri.azione} onChange={e => setFiltro('azione', e.target.value)}>
            <option value="">Tutte</option>
        ...
        {/* Reset */}
        <button
          onClick={() => setFiltri({ condominioId: '', categoria: '', azione: '', dataDal: '', dataAl: '', cerca: '', pagina: 0 })}
          style={{
            background: 'transparent', color: '#64748b', border: '1px solid #334155',
            borderRadius: 8, padding: '8px 14px', fontSize: 12, cursor: 'pointer',
            fontFamily: 'Sora, sans-serif', alignSelf: 'flex-end'
          }}
        >
          Reset
        </button>
      ...
      {/* Lista log */}
      {loading ? (
        <div style={{ textAlign: 'center', color: '#64748b', padding: 40 }}>Caricamento archivio...</div>
      ) : log.length === 0 ? (
        <div style={{
        ...
        <>
          <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
            {log.map(evento => {
              ...
              // Calcola campi cambiati per UPDATE
              ...
              return (
                ...
                >
                  ...
                  >
                    {/* Azione badge */}
                    <span style={{
                      background: azioneInfo.bg, color: azioneInfo.color,
                    ...
                    <span style={{ fontSize: 16, flexShrink: 0 }}>
                      {CATEGORIA_ICONS[evento.categoria] || '📁'}
                    </span>
                    ...
                    <div style={{ flex: 1, minWidth: 0 }}>
                      <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                        <span style={{ color: '#f1f5f9', fontSize: 13, fontWeight: 500 }}>
                          {evento.tabella_modificata.replace(/_/g, ' ')}
                        </span>
                        {evento.condomini?.nome && (
                    ...
                    <div style={{ color: '#64748b', fontSize: 12, textAlign: 'right', flexShrink: 0 }}>
                      ...
                      <span style={{ fontSize: 11 }}>
                        {new Date(evento.created_at).toLocaleTimeString('it-IT', { hour: '2-digit', minute: '2-digit' })}
                      </span>
                    </div>

                    {/* Expand icon */}
                    {hasDiff && (
                      <span style={{ color: '#475569', fontSize: 14, flexShrink: 0, transform: isExpanded ? 'rotate(90deg)' : 'none', transition: 'transform 0.2s' }}>
                        ›
                      </span>
                    )}
                  </div>
                  ...
                  {isExpanded && hasDiff && (
                    <div style={{
                      borderTop: '1px solid #334155', padding: '14px 16px',
                      background: '#0f172a'
                    }}>
          ...
          {totale > PER_PAGINA && (
            <div style={{ display: 'flex', justifyContent: 'center', gap: 10, marginTop: 24 }}>
              <button
                disabled={filtri.pagina === 0}
                onClick={() => setFiltri(f => ({ ...f, pagina: f.pagina - 1 }))}
                style={{
                  background: filtri.pagina === 0 ? '#0f172a' : '#1e293b',
                  color: filtri.pagina === 0 ? '#334155' : '#94a3b8',
                  border: '1px solid #334155', borderRadius: 8,
                  padding: '8px 16px', cursor: filtri.pagina === 0 ? 'not-allowed' : 'pointer',
                  fontFamily: 'Sora, sans-serif'
                }}
              >
                ← Precedente
              ...
              <button
                disabled={(filtri.pagina + 1) * PER_PAGINA >= totale}
                onClick={() => setFiltri(f => ({ ...f, pagina: f.pagina + 1 }))}
                style={{
                  background: (filtri.pagina + 1) * PER_PAGINA >= totale ? '#0f172a' : '#1e293b',
                  color: (filtri.pagina + 1) * PER_PAGINA >= totale ? '#334155' : '#94a3b8',
                  border: '1px solid #334155', borderRadius: 8,
                  padding: '8px 16px',
                  cursor: (filtri.pagina + 1) * PER_PAGINA >= totale ? 'not-allowed' : 'pointer',
                  fontFamily: 'Sora, sans-serif'
                }}
              >
                Successiva →
